...
function Install()
{
  ...
  URLKernel='https://github.com/ouyangmland/lotServer-1/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/ouyangmland/lotServer-1/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/ouyangmland/lotServer-1/raw/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "http://speedsvip.ueuo.com/lotsever.php?ver=${acce_ver}&mac=${Mac}"
  ...
  wget --no-check-certificate -qO "/usr/lib/systemd/system/lotserver.service" "https://raw.githubusercontent.com/ouyangmland/lotServer-1/master/lotserver.service"
